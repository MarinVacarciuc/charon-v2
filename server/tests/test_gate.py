...
def confirm(state, node="gate-in", n=3, t=T0, faces=None):
    """Feed n identical frames so the tracker commits, per gate.py's tracker.confirm_frames=3."""
    events = []
    for i in range(n):
        events = process_frame(state, node, faces or [known()], allow_all, t + timedelta(milliseconds=300 * i))
    return events
...
def test_decision_fires_once_the_tracker_commits_not_before():
    state = GateNodeState()
    e1 = process_frame(state, "gate-in", [known()], allow_all, T0)
    e2 = process_frame(state, "gate-in", [known()], allow_all, T0 + timedelta(milliseconds=300))
    assert e1 == [] and e2 == []
    e3 = process_frame(state, "gate-in", [known()], allow_all, T0 + timedelta(milliseconds=600))
    assert len(e3) == 1 and e3[0].kind == "decision" and e3[0].granted

def test_decision_does_not_refire_every_frame_for_the_same_standing_person():
    state = GateNodeState()
    confirm(state)
    events = process_frame(state, "gate-in", [known()], allow_all, T0 + timedelta(seconds=1))
...
def test_denied_policy_still_produces_a_decision_event():
    ...
    state2 = GateNodeState()
    events2 = confirm(GateNodeState())
    # now with a denying policy
    events3 = []
    st = GateNodeState()
    for i in range(3):
        events3 = process_frame(st, "gate-in", [known()], deny_all, T0 + timedelta(milliseconds=300 * i))
...
def test_empty_frame_clears_the_tracker_without_a_stray_decision():
    state = GateNodeState()
    confirm(state)
    events = process_frame(state, "gate-in", [], allow_all, T0 + timedelta(seconds=1))
    assert events == []
...
def test_denied_but_crossed_is_its_own_event():
    ...
    st = GateNodeState()
    for i in range(3):
        process_frame(st, "gate-in", [known()], deny_all, T0 + timedelta(milliseconds=300 * i))
...
def test_denied_but_crossed_only_applies_to_entry_never_to_exit():
    ...
    st = GateNodeState()
    for i in range(3):
        process_frame(st, "gate-out", [known()], deny_all, T0 + timedelta(milliseconds=300 * i))
...
def test_a_fresh_binding_that_keeps_being_reaffirmed_stays_fresh_past_the_original_window():
    ...
    for i in range(20):  # ~6s of continued confident recognition, no NEW commit needed
        process_frame(state, "gate-in", [known()], allow_all, T0 + timedelta(milliseconds=300 * i))
